...
9. **Inventory upsert loop**: for each valid row's resolved product, if an `Inventory` row exists, **add** the row's `QuantityAvailable` to the existing on-hand quantity (a vendor import row is an incoming shipment, so it's additive like `Restock`, not a snapshot like the `PUT` endpoint), and **overwrite `SafetyStockLevel`/`ProcurementLeadTimeDays` only if the row provides a value** (`QuantityReserved` is left untouched); otherwise insert a new `Inventory` row with those values and `QuantityReserved = 0`.
